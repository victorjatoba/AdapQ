...
/**
 * This class holds an item responseVector vector for an examinee and stores a count of the number of examinees with the same responseVector vector. It computes
 * the loglikelihood and the first derivative of the loglikelihood. These methods can be used for maximum likelihood (ML), Bayes modal (MAP), expected a
 * posteriori (EAP), and proportinal curve fitting (PCF) estimation of person ability in univariate item responseVector models. Note: The PCF method should only
 * be used for members of the Rasch family of item responseVector models.
 *
 * The order of item in the array irm MUST be the same as the order or item responses in the array responses.
 *
...
public class IrtExaminee implements UnivariateDifferentiableFunction {
	...
	/**
	 * this constructor allows a single instance to be used for estimating ability for multiple people by calling setResponseVector() prior to calling for
	 * amethod to compute ability.
	 *
	 * @param groupID
	 *            examinee group ID
	 * @param irm
	 *            an array of item response models
	 */
	...
	/**
	 * If the response vector was not set in teh constructor, this method must be called prior to estimating ability.
	 *
	...
	public boolean isExtreme() {
		if (sumScore == MinPRS || sumScore == MaxPRS)
			return true;
		return false;
	}
	...
	public double logLikelihood(double theta) {
		if (responseVector.getValidResponseCount() <= 0)
			return Double.NaN;
		double ll = 0.0;
		double prob = 0.0;
		byte resp = 0;
	 ...
	 *
	 * @param thetaMin
	 *            smallest possible ability estimate (lower bound on BrentOptimizer)
	 * @param thetaMax
	 *            largest possible ability estimate (upper bound on BrentOptimizer)
	...
	public double maximumLikelihoodEstimate(double thetaMin, double thetaMax, int maxIter, double tolerance) {
		...
		UnivariatePointValuePair pair = optimizer.optimize(new MaxEval(maxIter), new UnivariateObjectiveFunction(this), GoalType.MAXIMIZE, new SearchInterval(
				thetaMin, thetaMax));
	 ...
	 * @param thetaMin
	 *            smallest possible ability score
	 * @param thetaMax
	 *            largest possible ability score
	 * @param numPoints
	 ...
	 *
	 * @param dist
	 *            User specified quadrature points and weights.
	 * @return
	 */
	...
	/**
	 * Computes ability estimate using proportional curve fitting. This method is only appropriate with the Rasch, Partial Credit, and Rating Scale models. It
	 * may not converge to the maximum value if used with other models. This method is the same method used for person estimation in the Rasch package.
	 *
	 * @param maxIter
	 *            maximum number of iterations
	 * @param converge
	 *            convergence criterion (e.g. 0.01)
	 * @param adjustment
	 *            extreme score (i.e. all items correct) adjustment factor

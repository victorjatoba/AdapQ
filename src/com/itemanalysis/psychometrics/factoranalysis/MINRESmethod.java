...
public class MINRESmethod extends AbstractFactorMethod {
	...
	// TODO method changed by @victor.jatoba
	public double estimateParameters() {
		...
		// new SimpleValueChecker(1e-10, 1e-10), 1e-4, 1e-4, 1);
		...
		solution = optimizer.optimize(new MaxEval(1000),
				...
				GoalType.MINIMIZE, new InitialGuess(getStartValues()));
	...
	private void computeFactorLoadings(double[] x) {
		...
		RealMatrix L = E.getV()
				.getSubMatrix(0, nVariables - 1, 0, nFactors - 1);
		...
		for (int j = 0; j < nFactors; j++) {
			proportionOfExplainedVariance[j] = sumsOfSquares[j]
					/ totSumOfSquares;
	...
	public double[] getStartValues() {
		...
		if (nFactors == 1) {
			...
			RealMatrix rInverse = new LUDecomposition(R2).getSolver()
					.getInverse();
			for (int i = 0; i < nVariables; i++) {
	...
	private class MINRESObjectiveFunction extends AbstractMultivariateFunction {
		...
		public double valueAt(double[] param) {
			...
			RealMatrix eigenVectors = eigen.getV().getSubMatrix(0,
					nVariables - 1, 0, nFactors - 1);

			double[] ev = new double[nFactors];
			for (int i = 0; i < nFactors; i++) {
				ev[i] = Math.sqrt(eigen.getRealEigenvalue(i));
			}
			DiagonalMatrix evMatrix = new DiagonalMatrix(ev);// USE Apache
																// version of
																// Diagonal
																// matrix when
																// upgrade to
		...
		public double[] gradientAt(double[] x) {
			...
			double[] invSqrtPsi = new double[nVariables];
			for (int i = 0; i < nVariables; i++) {
				x[i] = Math.max(0.005, x[i]);// ensure that no parameters are
												// negative
				sqrtPsi[i] = Math.sqrt(x[i]);
				invSqrtPsi[i] = 1.0 / Math.sqrt(x[i]);
			}
			...
			EigenDecomposition E = new EigenDecomposition(Sstar);
			RealMatrix L = E.getV().getSubMatrix(0, nVariables - 1, 0,
					nFactors - 1);
			double[] ev = new double[nFactors];
			for (int i = 0; i < nFactors; i++) {
				ev[i] = Math.sqrt(Math.max(E.getRealEigenvalue(i) - 1, 0));
			...
			RealMatrix G = LL.multiply(LL.transpose()).add(diagPsi)
					.subtract(R2);
